...
FNAME = "E" + str(NEVENTS) + "S" + str(NSAMPLES) + "ASF" + str(ASD_SCALE_FACTOR)+".h5"
...
if __name__ == "__main__":
    ...
    with h5py.File(FNAME, "w") as f:#SNRwoth1000S1e4
        for i in range(NEVENTS):
            ...
            print (i,z_true,theta_true,snr)
            #print (fisher[:-2,:-2])   
            CoV=np.linalg.inv(fisher[:-2,:-2])
            f.create_dataset('true_parameters'+str(i),data=(np.array([m1_true, m2_true, z_true, np.log(deff_true)])))
...
##############################################################################            
            ...
            if (MAKE_LIKELIHOOD_SAMPLES):
                #print (m1_measured[0], m2_measured[0], z_measured[0], log_deff_measured[0])
